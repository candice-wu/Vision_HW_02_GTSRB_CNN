...
本系統採用了兩種主要的建模策略進行比較：
1. **深度學習 (CNN + STN)**：
   我們實作了一個卷積神經網路 (CNN)，並在最前方加入了一個**空間轉換網路 (Spatial Transformer Network, STN)**。STN 模組負責學習圖像的仿射變換 (Affine Transformation)，能在特徵提取前，先將圖像中的交通號誌進行縮放、旋轉或平移校正，減少因拍攝角度與位置造成的干擾。隨後的 CNN 由三個捲積層 (Conv2d) 與最大池化層 (MaxPool) 組成，最後連接 Fully Connected 層進行 43 類的分類。
...
### 1-2. 損失函數說明
在 CNN 的訓練中，我們採用了 **Cross Entropy Loss (交叉熵損失函數)**。
...
透過 `src/evaluate.py` 產出的各類圖表，我們進行了以下評估：
...
2. **機器學習特徵降維**：
   針對傳統 ML，我們在建模前強制加入了 PCA 管線，大幅降低特徵維度，使得 SVM 與 AdaBoost 的訓練能在合理時間內完成。

### 3-3. 技術反思
本次實驗清楚展現了「特徵工程」的演進。在傳統 ML 時代，我們需要先做 PCA 等複雜的前處理，且因為打平影像遺失了 2D 空間資訊，導致準確率存在天花板。相對地，CNN 能夠自動學習空間特徵濾波器，而 STN 更是讓模型具備了空間變換的抗性，這是深度學習在電腦視覺上無法被取代的強大優勢。最後，將所有模型部署至 Streamlit，實現了可視化的成果展示，大幅提升了模型的實用性與互動性。
...
2. **STN 依賴的 grid_sample 與 Mac MPS 支援問題**
   ...
   * **解法**：我們在程式起始處設定了環境變數 `os.environ['PYTORCH_ENABLE_MPS_FALLBACK'] = '1'`。這項設定能強制將 MPS 尚未支援的運算子退回給 CPU 處理，其餘運算依舊保留在 GPU 進行，從而完美避開了報錯並維持了訓練效率。
